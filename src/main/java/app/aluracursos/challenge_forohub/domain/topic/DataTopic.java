...
public record DataTopic(Long topicoId, String titulo, String mensaje, LocalDateTime fechaDeCreacion, Long usuarioId, String nombre, Cursos curso,

        List<DataLimitResponse> respuestas
) {
    public DataTopic(Topic topic, UserRepository uRepository, ResponsesRepository rRepository) {
        this(topic.getId(), topic.getTitulo(), topic.getMensaje(), topic.getFechaDeCreacion(), topic.getUser().getId(), uRepository.getReferenceById(topic.getUser().getId()).getNombre(),
                topic.getCurso(), rRepository.findAllByTopicoId(topic.getId()).stream().map(r -> new DataLimitResponse(r.getId(), uRepository.getReferenceById
